...
def preprocess_image(path=None, size=256, border_ratio=0.2, recenter=True, dont_recenter=None, image=None):
    parser = argparse.ArgumentParser()
    parser.add_argument('path',default=path, type=str, help="path to image (png, jpeg, etc.)")
    parser.add_argument('--size', default=256, type=int, help="output resolution")
    parser.add_argument('--border_ratio', default=0.2, type=float, help="output border ratio")
    parser.add_argument('--recenter', type=bool, default=True, help="recenter, potentially not helpful for multiview zero123")
    parser.add_argument('--dont_recenter', dest='recenter', action='store_false')
    ...
    opt.path = path
    opt.size = size
    opt.border_ratio = border_ratio
    opt.recenter = recenter 
    opt.dont_recenter = dont_recenter
